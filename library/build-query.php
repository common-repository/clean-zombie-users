...
if (isset($_POST['page_updated'])) { // if the post has been updated
    ...
    foreach ($roles as $key => $role) { // iterate over detected roles
        $getcheck = "include-" . $key; // get the role as a value that matches the form as a variable for complex inclusion
        if(isset($_POST["{$getcheck}"])) { ${"include_" . $key} = (bool)$_POST["{$getcheck}"]; } // set a dynamic variable and check it against the submitted form
        if (isset(${"include_" . $key})) { // if the matching role is checked in the form
            $role_options[] = "$wpdb->usermeta.meta_value LIKE '%" . $key . "%' "; // generate an array of statements for inclusion
        }
    }
    ...
    if (isset($_POST['never-post'])) {
        $never_post = (bool)$_POST['never-post'];
    }
    if (isset($_POST['never-comment'])) {
