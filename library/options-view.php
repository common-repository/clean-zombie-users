...
<div id=clean-wrap>
    ...
    <form id="zombie-cage" enctype="multipart/form-data" method="post"
        ...
        <p style="color: #c2403a;"><em><strong>DO NOT use this on a commerce site or you may delete customers</strong>
                who have not technically commented on a blog entry or written a post.</em></p>
        ...
        <p>Do you have a slow database? If so, you may want to do this in chunks. You can limit the number of results
            processed at once with this setting. Highly recommended for the actual deletion, up to you for testing.</p>
        <ul>
        ...
        <div id="the-options">
            ...
            foreach ($roles as $key => $role) {
                ...
                /**
                 * A function to break up weird user role names on some older sites.
                 * For example, "Subscriber" will be named "Subscriber|User Roles",
                 * which looks strange when generated as a role name for processing.
                 */

                $thisrole = $role['name']; // Assign the role name to a variable for processing

                if (preg_match("/\|/", $thisrole) == true) :    // Test for a piped role name
                    $exploded = explode("|", $thisrole);        // If found, explode the parts
                    $thisrole = $exploded[0];                   // Reset the role name to keep only the first value
                endif;

                echo "<input type='checkbox' value='include-" . $key . "' name='include-" . $key . "' />";
                echo "<label name='include-" . $key . "' >" . $thisrole . "s</label><br/>";
            }
            ?>
    ...
    <p>If an author with no posts shows up here, it means they have active revisions. Often this is your webmaster but
        sometimes it's a SQL injection or administrative error. Make sure everyone on this list is authorized and
        delete/modify any mystery authors!</p>
